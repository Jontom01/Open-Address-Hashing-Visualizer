...
class OAHashTable{
    ...
    async #insertProbe(key, flag){
        ...
        while(i < this.#tableSize){
            steps++;
            //chooses which slot/box in the table to check
            let check = this.#multiplicationHash(key, flag, i);
            //If colour is white then proceed
            ...
            //else if it is not white
                ...
                //temporarily changes colour to grey to make it look like the algorithm is moving over that value
                ...
                //changes colour back to original (this used to be an else statement from the previous commented out if statement)
                this.#table[check].style.backgroundColor = "rgb(85, 214, 170)";
    ...
    insertLinear(key){
        return this.hashingAlgorithms.linearProbe(key, table);
    }
    insertQuadratic(key){
